...
// Modeled after OmniPost's services/ai/prompts.ts

import type { Platform, WritingStyleGuide } from '@cobrain/core'

/**
 * Build a system prompt for content generation.
 */
export function buildSystemPrompt(
  ...
  const styleInstructions = style
    ? buildStyleInstructions(style, platform)
    : ''
...
/**
 * Build a user prompt from source notes.
 */
...
function buildStyleInstructions(
  ...
  const override = style.serviceOverrides?.[platform]
...
- Format: Start with title as # heading, include frontmatter-style metadata at the end`,
