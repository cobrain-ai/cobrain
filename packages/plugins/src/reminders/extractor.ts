...
export function extractBasicReminders(text: string): ExtractedReminder[] {
  ...
  const remindPattern =
    /remind(?:er)?(?:\s+me)?\s+(?:to\s+)?(.+?)(?:\s+at\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?)?(?:\s+(?:on\s+)?(tomorrow|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday))?/gi
  ...
  const commitmentPattern =
    /i(?:'ll| will)\s+(.+?)\s+by\s+(tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday|next week|end of (?:the )?week)/gi

  while ((match = commitmentPattern.exec(text)) !== null) {
    const message = match[1]?.trim()
    if (!message) continue

    const triggerAt: Date = new Date(now)
  ...
  const recurringPattern =
    /every\s+(day|morning|evening|monday|tuesday|wednesday|thursday|friday|saturday|sunday|week|month)\s*(?:at\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?)?\s*[,:]?\s*(.+)?/gi
  ...
  while ((match = recurringPattern.exec(text)) !== null) {
    ...
    let cronPattern = ''
    if (frequency === 'day') cronPattern = '0 9 * * *' // Daily at 9am
    else if (frequency === 'morning') cronPattern = '0 8 * * *'
    else if (frequency === 'evening') cronPattern = '0 18 * * *'
    else if (frequency === 'week') cronPattern = '0 9 * * 1' // Weekly Monday
    else if (frequency === 'month') cronPattern = '0 9 1 * *' // Monthly 1st
    else {
      // Day of week
      const days = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday']
      const dayIndex = days.indexOf(frequency)
      if (dayIndex !== -1) {
        cronPattern = `0 9 * * ${dayIndex}` // That day at 9am
      }
    }
